...
enum Inet_Ret_Code socket_connect(int _socket) {
	dprintf(inet_log_fd, "connecting socket %d", _socket);
	int st = connect(_socket, (struct sockaddr *)&GlobalInetBus.addr,
	            sizeof(GlobalInetBus.addr));
	if (st == 0) {
		dprintf(inet_log_fd, "socket %d connected", _socket);
		return INET_SUCCESS;
	} else {
		dprintf(inet_log_fd, "failed to connect socket %d", _socket);
...
enum Inet_Ret_Code read_data(int _socket, char* _buffer, size_t _size, size_t _timeout_sec) {
	...
	dprintf(inet_log_fd,
			"reading data over socket=%d",_socket); 
